...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  try {
    ...
    if (!args[0] && event.messageReply.attachments.length == 0)
      return api.sendMessage(formatMessage(`You haven't entered what to report 📋`),
        event.threadID,
        event.messageID
      );
    ...
    api.sendMessage(formatMessage(`[🤖] - Bot has just successfully sent your message to ${soad} to my owner 𝐏𝐫𝐢𝐲𝐚𝐧𝐬𝐡 𝐑𝐚𝐣𝐩𝐮𝐭 🍄 \n[⏰] - Time: ${gio}`),
      event.threadID,
      () => {
        var idad = global.config.ADMINBOT;
        if (s.length == 0) {
