...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                throw new Error('Could not load config module, error: ' + JSON.stringify(error));
            }
    ...
    if (errorList.length != 0) api.sendMessage(formatMessage('Commands that have just crashed when the system loads:\n' + errorList.join('\n')), threadID, messageID);
    api.sendMessage(formatMessage('The Bot system has just loaded successfully ' + (moduleList.length - errorList.length) +' command\nLoaded modules: '+moduleList.join(', ') + '.js'), threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(formatMessage(`Successfully canceled ${moduleList.length} command`), threadID, messageID);
}
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (args[0]) {
      ...
      let commands = client.commands.values();
                  let infoCommand = "";
                        api.sendMessage(formatMessage("Currently includes " + client.commands.size + " Usable commands" + infoCommand), event.threadID, event.messageID);
      break;
                }
        case "load": {
            if (moduleList.length == 0) return api.sendMessage(formatMessage("Module name does not allow blank"), threadID, messageID);
        ...
        case "info": {
            ...
            if (!command) return api.sendMessage(formatMessage("The module you imported does not exist"), threadID, messageID);
            ...
            return api.sendMessage(
                formatMessage(
                    ...
                    "- Version: " + version + "\n" +
                    "- Request permissions: " + ((hasPermssion == 0) ? "User" : (hasPermssion == 1) ? "Admin" : "Support" ) + "\n" +
