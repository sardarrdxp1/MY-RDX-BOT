...
module.exports.handleReply = async ({ handleReply, event, api }) => {
  ...
  else return api.sendMessage(formatMessage("Please choose <add | del > <numerical order | or \"all\">"), event.threadID, event.messageID);
  ...
  api.sendMessage(formatMessage(`» Đã ${args[0] == 'add' ? 'accept' : 'erase'} successful friend request of ${success.length} People:\n${success.join("\n")}${failed.length > 0 ? `\n» Fail with ${failed.length} People: ${failed.join("\n")}` : ""}`), event.threadID, event.messageID);
};
