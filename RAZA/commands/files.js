...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  api.sendMessage(formatMessage("⚡️Deleted the following files in the commands folder:\n\n"+msg, event.threadID, event.messageID);
}
...
•Example: commands help`;
    ...
    if(files.length == 0) return api.sendMessage(formatMessage(`⚡️There are no files in the cache that begin with: ${word}`), event.threadID ,event. messageID);
  ...
  //đuôi file là..... 
    ...
    if(files.length == 0) return api.sendMessage(formatMessage(`⚡️There are no files in commands that end with: ${ext}`), event.threadID ,event. messageID);
    var key = `⚡️Có ${files.length} file có đuôi là: ${ext}`;
  }
  //all file
  else if (!args[0]) {
  if(files.length == 0) return api.sendMessage(formatMessage("⚡️Your commands have no files or folders"), event.threadID ,event. messageID);
  ...
  //trong tên có ký tự.....
    ...
    if(files.length == 0) return api.sendMessage(formatMessage(`⚡️There are no files in the name with the character: ${word}`), event.threadID ,event. messageID);
