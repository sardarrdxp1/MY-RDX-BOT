...
module.exports.handleSchedule = async ({ api, schedule }) => {
    try {
        ...
        return api.sendMessage(formatMessage(`[Outbox] has left the group ID ${schedule.target}`), __GLOBAL.settings.ADMINBOT[0], (error, info) => {
            if (error) return require(process.cwd() + "/utils/log")(`Has left the group ID ${schedule.target}`, "[ OUTBOX ]");
        });
    }
    catch {
        return api.sendMessage(formatMessage(`Could not leave the group ID ${schedule.target}!`), __GLOBAL.settings.ADMINBOT[0], (error, info) => {
...
module.exports.handleReply = ({ event, api, handleReply }) => {
    ...
    switch (handleReply.type) {
        case "inputThreadID": {
            if (isNaN(event.body)) return api.sendMessage(formatMessage("[Outbox] ThreadID you enter inappropriate format!"), event.threadID, event.messageID);
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(formatMessage(`[Outbox] The time you need a timer (note must be in the format: HH:mm):`), event.threadID, (error, info) => {
        ...
        case "inputTime": {
            ...
            if (!regex.test(event.body)) return api.sendMessage(formatMessage(`[Outbox] Format time incorrectly!`), event.threadID, event.messageID);
            ...
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(formatMessage(`[Outbox] Enter the reason automatically removed:`), event.threadID, (error, info) => {
                global.client.handleReply.push({
                    type: "inputReason",
                    name: this.config.name,
                    author: event.senderID,
        ...
        case "inputReason": {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(formatMessage("=== OutBox ===" +
...
module.exports.run = ({  event, api }) => {
    return api.sendMessage(`[Outbox] Thread ID You need a loose automatic timer:`, event.threadID, (error, info) => {
        global.client.handleReply.push({
            type: "inputThreadID"), "inputThreadID",
            name: this.config.name,
            author: event.senderID,
            messageID: info.messageID
        })
