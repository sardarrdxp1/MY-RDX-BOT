...
module.exports.handleReply = ({ api, event, args, handleReply }) => {
  ...
  api.sendMessage(formatMessage("Deleted the following files in the cache folder:\n\n"+msg, event.threadID, event.messageID);
}
...
•Example: Cache Help`;
    ...
    if(files.length == 0) return api.sendMessage(formatMessage(`There are no files in the cache that begin with: ${word}`), event.threadID ,event. messageID);
  ...
  //The file extension is..... 
  	...
  	if(files.length == 0) return api.sendMessage(formatMessage(`There are no files in the cache with a character ending in .: ${ext}`), event.threadID ,event. messageID);
  	var key = `Có ${files.length} file có đuôi là: ${ext}`;
  }
  //all file
  else if (!args[0]) {
  if(files.length == 0) return api.sendMessage(formatMessage("Your cache has no files or folders"), event.threadID ,event. messageID);
  ...
  //trong tên có ký tự.....
  	...
  	if(files.length == 0) return api.sendMessage(formatMessage(`There are no files in the name with the character: ${word}`), event.threadID ,event. messageID);
